...
func TestUser_Delete(t *testing.T) {
	...
	if err == nil {
		t.Error("retrieved user thats deleted")
	}
}
...
func TestToken_GetUserForToken(t *testing.T) {
	...
	if err == nil {
		t.Error("err expected but not recieved when getting user with a bad token")
	}
...
func TestToken_GetByToken(t *testing.T) {
	...
	if err == nil {
		t.Error("no error getting nonexistent token by token: ", err)
	}
}
...
func TestToken_ValidToken(t *testing.T) {
	...
	if okay {
		t.Error("no error reported when validing non existent token")
	}
}
